...
router.get('/', async (req: Request, resp: Response, next: NextFunction) => {
    try {
        const queryHandlings = await QueryHandlingService.findAll();
...
router.get('/:id', async (req: Request, resp: Response, next: NextFunction) => {
    try {
        const existingQueryHandling = await QueryHandlingService.findById(
            req.params.id
        );
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        const data = await QueryHandlingService.update(queryHandlingId, {
            ...req.body,
        });
...
router.delete('/:id', async (req, res, next) => {
    try {
        const queryHandlingId = req.params.id;
        await QueryHandlingService.deleteByPrimaryKey(queryHandlingId);
...
router.post('/', async (req: Request, resp: Response, next: NextFunction) => {
    try {
        ...
        const newQueryHandling = await QueryHandlingService.save(payload);
